...
Nozdormu 是基于 Cloudflare Pingora 框架构建的高性能 CDN 反向代理，从 OpenResty/Lua 栈迁移而来。当前已实现 9 个 crate、554 个单元/集成测试、79 个 E2E 测试。核心能力包括：WAF（IP/GeoIP/ASN/Body 检测）、CC 限速（混合本地+Redis 计数）、缓存（OSS/S3 + Redis 元数据 + SWR + Request Coalescing + Cache Tags + 缓存预热）、SSL/TLS（完整 ACME v2 自动签发与续期 + TLS 1.3 0-RTT）、多协议支持（WebSocket/SSE/gRPC）、动态配置（etcd 热加载 + 配置版本管理与回滚）、视频流优化（URL 签名/MP4→HLS 动态封装/LL-HLS 部分分片/智能预取）、**直播接入**（RTMP/SRT 推流 → HLS/LL-HLS 实时分段 + 内存环形缓冲 + Stream Key 认证）、图片优化（实时 resize/格式转换）、负载均衡（4 种算法 + 自适应权重）、多后端多通道日志（Redis/Kafka/RabbitMQ/NATS/Pulsar × 8 通道）。
...
| 7 | Webhook 事件通知 | 定制化 | 3 天 |
| 8 | API 版本化 (v1) | 定制化 | 2 天 |
| 9 | Tiered Cache（分层缓存） | 缓存 | 2-3 周 |

### 🟢 低优先级（锦上添花 / 长期规划）

| # | 差距项 | 维度 | 备注 |
|---|--------|------|------|
| 10 | 边缘函数 (Wasm) | 边缘计算 | 平台化方向，需大量投入 |
| 11 | 灰度发布 | 运维 | 配置级流量分割 |
| 12 | Terraform Provider | 定制化 | IaC 生态 |
| 13 | 多语言 SDK | 定制化 | Go/Python/JS |
| 14 | mTLS 客户端证书 | 安全 | API 网关场景 |
| 15 | L3/L4 DDoS 防护 | 安全 | 需网络层基础设施 |
| 16 | 自适应码率 (ABR) | 媒体 | Master Playlist + 多码率 Variant |
...
- ✅ **工程质量**：554 单元测试 + 79 E2E 测试 + CI/CD + OpenAPI 文档
