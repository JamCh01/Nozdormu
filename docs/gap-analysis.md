...
Nozdormu 是基于 Cloudflare Pingora 框架构建的高性能 CDN 反向代理，从 OpenResty/Lua 栈迁移而来。当前已实现 8 个 crate、509 个单元/集成测试、79 个 E2E 测试。核心能力包括：WAF（IP/GeoIP/ASN/Body 检测）、CC 限速（混合本地+Redis 计数）、缓存（OSS/S3 + Redis 元数据 + SWR + Request Coalescing + Cache Tags + 缓存预热）、SSL/TLS（完整 ACME v2 自动签发与续期 + TLS 1.3 0-RTT）、多协议支持（WebSocket/SSE/gRPC）、动态配置（etcd 热加载 + 配置版本管理与回滚）、视频流优化（URL 签名/MP4→HLS 动态封装/LL-HLS 部分分片/智能预取）、图片优化（实时 resize/格式转换）、负载均衡（4 种算法 + 自适应权重）、多后端多通道日志（Redis/Kafka/RabbitMQ/NATS/Pulsar × 8 通道）。
...
| LL-HLS | ✅ `#EXT-X-PART` 部分分片 + `#EXT-X-PART-INF` + `#EXT-X-SERVER-CONTROL`（CAN-BLOCK-RELOAD, PART-HOLD-BACK）+ `INDEPENDENT=YES` 关键帧标记 | 低延迟 HLS | **已完成**（VOD） |
| 智能预取 | ✅ HLS/DASH manifest 解析 + 后台预取 | 高级功能 | 无 |
| **直播接入** | ❌ 无 RTMP/SRT 接入（架构已确认：CDN 内置，新增 cdn-ingest crate） | RTMP/SRT 推流接入 + 实时分段 | **缺失** |
...
- **中优先**：实现 **RTMP/SRT 直播接入**。新增 `cdn-ingest` crate，在 cdn-proxy 进程内增加 RTMP(1935)/SRT(UDP) 监听器，接收推流后实时分段为 HLS/LL-HLS，通过现有 HTTP 代理分发。
...
| 维度 | 项目现状 | 现代化 CDN 标准 | 差距 |
|------|---------|----------------|------|
| Prometheus 指标 | ✅ 25+ Counter/Histogram/Gauge，覆盖全链路（请求/上游/健康检查/缓存/图片/流媒体/0-RTT/ACME/SWR/Coalescing/预热/自适应权重） | 优秀 | 无 |
...
| 图片处理 | ✅ 实时优化 + 格式协商 | 高级 | 无 |
| **直播** | ❌ 无 RTMP/SRT 接入（LL-HLS VOD 已完成，直播接入架构已确认） | 低延迟直播（WebRTC/LL-HLS）、连麦、转推 | **部分缺失** |
...
- **如果面向视频业务**：下一步实现 **RTMP/SRT 直播接入**（cdn-ingest crate，CDN 内置架构），将 LL-HLS 从 VOD 扩展到直播场景。
...
| 2 | **自定义错误页** | 定制化 | 3 天 | SiteConfig 扩展 |
| 3 | **HTTP/3 (QUIC)** | 协议 | 取决于 Pingora 上游 | Pingora QUIC 稳定性 |

### 🟡 中优先级（提升产品完整度）

| # | 差距项 | 维度 | 预估工作量 |
|---|--------|------|-----------|
| 4 | Bot 管理（UA 分类 + 行为检测） | 安全 | 2 周 |
| 5 | OpenTelemetry 分布式追踪 | 可观测 | 1-2 周 |
| 6 | DASH 动态封装 | 媒体 | 1-2 周 |
| 7 | RTMP/SRT 直播接入 | 媒体 | 3-4 周 |
| 8 | Webhook 事件通知 | 定制化 | 3 天 |
| 9 | API 版本化 (v1) | 定制化 | 2 天 |
| 10 | Tiered Cache（分层缓存） | 缓存 | 2-3 周 |

### 🟢 低优先级（锦上添花 / 长期规划）

| # | 差距项 | 维度 | 备注 |
|---|--------|------|------|
| 11 | 边缘函数 (Wasm) | 边缘计算 | 平台化方向，需大量投入 |
| 12 | 灰度发布 | 运维 | 配置级流量分割 |
| 13 | Terraform Provider | 定制化 | IaC 生态 |
| 14 | 多语言 SDK | 定制化 | Go/Python/JS |
| 15 | mTLS 客户端证书 | 安全 | API 网关场景 |
| 16 | L3/L4 DDoS 防护 | 安全 | 需网络层基础设施 |
| 17 | 自适应码率 (ABR) | 媒体 | Master Playlist + 多码率 Variant |
...
- ✅ **视频流优化**：MP4→HLS 动态封装 + LL-HLS 部分分片 + 智能预取 + 3 种 URL 签名，功能完整
...
- ✅ **可观测性**：25+ Prometheus 指标 + 5 后端 × 8 通道日志 + 4 阶段计时
- ✅ **配置管理**：ArcSwap 热加载 + etcd watch + 版本管理与一键回滚
- ✅ **工程质量**：509 单元测试 + 79 E2E 测试 + CI/CD + OpenAPI 文档

### 核心差距（需优先补齐）

- ❌ **WAF 规则薄弱**：仅有 IP/GeoIP 规则，缺少 OWASP Top 10 防护
- ❌ **HTTP/3 缺失**：移动端和弱网环境的关键加速协议（依赖 Pingora 上游）
- ❌ **直播接入缺失**：无 RTMP/SRT 推流接入（LL-HLS VOD 已就绪，架构已确认）
...
| 媒体处理 | 85/100 | 图片+视频+LL-HLS，缺直播接入 |
| 可观测性 | 85/100 | 多后端多通道日志+指标完整，缺追踪 |
| 边缘计算 | 0/100 | 无 |
| 运维自动化 | 80/100 | 热加载+版本管理+回滚，缺灰度 |
| **综合** | **73/100** | 核心功能全面，主要差距在 WAF 规则和 QUIC |
