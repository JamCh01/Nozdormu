...
| **最少连接 LB** | ❌ 未实现 | 基于活跃连接数的负载均衡 | **缺失** |
| **一致性哈希 LB** | ❌ 仅 DJB2 IP Hash | Ketama 一致性哈希，节点变更时最小化缓存失效 | **缺失** |
| **动态权重调整** | ❌ 权重静态配置 | 基于响应时间/错误率自动调整权重 | **缺失** |
| **多源站 failover** | ✅ backup origin 支持 | 基线 | 无 |

**优先级：中**

**优化建议：**
- **中优先**：增加 **Least Connections** 算法。在 `balancer.rs` 中增加 `LbAlgorithm::LeastConn`，使用 `AtomicU32` 跟踪每个 origin 的活跃连接数。
- **中优先**：增加 **一致性哈希**。替换 DJB2 为 Ketama 或 Jump Consistent Hash，确保 origin 增减时只有 1/N 的请求重新映射。
- **低优先**：实现 **动态权重**。基于滑动窗口内的 P99 延迟和错误率，自动调整 origin 权重（类似 Envoy 的 Zone-Aware Load Balancing）。
