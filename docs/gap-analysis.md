...
Nozdormu 是基于 Cloudflare Pingora 框架构建的高性能 CDN 反向代理，从 OpenResty/Lua 栈迁移而来。当前已实现 9 个 crate、568 个单元/集成测试、79 个 E2E 测试。核心能力包括：WAF（IP/GeoIP/ASN/Body 检测）、CC 限速（混合本地+Redis 计数）、缓存（OSS/S3 + Redis 元数据 + SWR + Request Coalescing + Cache Tags + 缓存预热）、SSL/TLS（完整 ACME v2 自动签发与续期 + TLS 1.3 0-RTT）、多协议支持（WebSocket/SSE/gRPC）、动态配置（etcd 热加载 + 配置版本管理与回滚）、视频流优化（URL 签名/MP4→HLS 动态封装/LL-HLS 部分分片/智能预取）、**直播接入**（RTMP/SRT 推流 → HLS/LL-HLS 实时分段 + 内存环形缓冲 + Stream Key 认证）、图片优化（实时 resize/格式转换）、负载均衡（4 种算法 + 自适应权重）、多后端多通道日志（Redis/Kafka/RabbitMQ/NATS/Pulsar × 8 通道）、**自定义错误页**（per-site per-status-code HTML 错误页面）、**Webhook 通知**（证书续期/健康变更/缓存清除事件 → 配置的 webhook URL，HMAC 签名 + 重试）。
...
| **Webhook 通知** | ✅ 证书续期/健康变更/缓存清除事件 → 配置的 webhook URL，HMAC-SHA256 签名，指数退避重试 | 证书到期、健康状态变更、缓存刷新完成等事件通知 | **已完成** |
...
- ~~**中优先**：增加 **Webhook 通知**。~~ ✅ 已完成（`WebhookConfig` 全局配置，`mpsc::channel(256)` 事件通道，3 个事件源：证书续期/健康变更/缓存清除，HMAC-SHA256 签名，指数退避重试，`WebhookDeliveryTracker` 投递追踪，管理 API `GET /_admin/webhook/events` + `POST /_admin/webhook/test`）
...
| 15 | ~~Webhook 事件通知~~ | 定制化 | ✅ 全局 WebhookConfig，mpsc 事件通道，3 事件源（证书/健康/缓存清除），HMAC 签名 + 重试，管理 API |
...
| 6 | API 版本化 (v1) | 定制化 | 2 天 |
| 7 | Tiered Cache（分层缓存） | 缓存 | 2-3 周 |
...
- ✅ **工程质量**：568 单元测试 + 79 E2E 测试 + CI/CD + OpenAPI 文档
